...
drop policy if exists "fornecedores: member insert" on public.fornecedores;
create policy "fornecedores: member insert"
  on public.fornecedores for insert
  with check (public.user_belongs_to_empresa(empresa_id));

drop policy if exists "fornecedores: member update" on public.fornecedores;
create policy "fornecedores: member update"
  on public.fornecedores for update
  using (public.user_belongs_to_empresa(empresa_id));

drop policy if exists "fornecedores: member delete" on public.fornecedores;
...
do $$ begin
  create type public.status_envio as enum ('enviado', 'erro', 'pendente');
exception
  when duplicate_object then null;
end $$;

create table if not exists public.envios_pmg (
...
create index if not exists idx_envios_pmg_empresa on public.envios_pmg (empresa_id, created_at desc);

alter table public.envios_pmg enable row level security;

drop policy if exists "envios_pmg: member read" on public.envios_pmg;
create policy "envios_pmg: member read"
  on public.envios_pmg for select
  using (public.user_belongs_to_empresa(empresa_id));

drop policy if exists "envios_pmg: member insert" on public.envios_pmg;
